...
- Tasks:
    ...
    - Step 9 - Changing group ownership for {{ showTooltip("PostgreSQL directories", "/opt; /opt/mount_point") }}
    - Step 10 - Setting permissions for {{ showTooltip("PostgreSQL directories", "/opt; /opt/mount_point") }}
    ...
    - Step 15 - Deleting the {{ showTooltip("old main Postresql directory", "/var/lib/postgresql/14/main") }} directory
    ...
    - Step 18 - Adding permissions to **pg_hba.conf** in ``/etc/postgresql/14/main/pg_hba.conf`` for connecting to the PostgreSQL {{ showTooltip("**Important note!**", "Perform only on the MediaWiki db's") }}
    - Step 19 - Restarting the PostgreSQL service
    - Step 20 - Creating the ``/scripts`` directory with ``0755`` permissions
    - Step 21 - Creating the **.env** file in the ``/scripts/.env`` directory with ``0740`` permissions
    - Step 22 - Copying the **pgdump_standby_postgresql.py** script to ``/scripts/pgdump_standby_postgresql.py`` directory with ``0755`` permissions {{ showTooltip("**Important note!**", "Perform only on the MediaWiki db's") }}
    - Step 23 - Filling the **.env** file in ``/scripts/.env`` directory with data {{ showTooltip("**Important note!**", "Perform only on the MediaWiki db's") }}
    - Step 24 - Copying the {{ showTooltip("**id_ed25519**", "Private ssh-key") }} file to ``~/.ssh/id_ed25519`` with ``600`` permissions
    - Step 25 - Copying the **logrotate_pgdump_standby_posgresql** file to ``/etc/logrotate.d/logrotate_pgdump_standby_posgresql`` {{ showTooltip("**Important note!**", "Perform only on the MediaWiki db's") }}
    - Step 26 - Starting and enabling the logrotate service {{ showTooltip("**Important note!**", "Perform only on the MediaWiki db's") }}
...
- Step 18 - Adding permissions to {{ showTooltip_Direction("**pg_hba.conf**", "/etc/postgresql/14/main/pg_hba.conf") }} for connecting to the PostgreSQL {{ showTooltip("**Important note!**", "Perform only on the MediaWiki db's") }}
