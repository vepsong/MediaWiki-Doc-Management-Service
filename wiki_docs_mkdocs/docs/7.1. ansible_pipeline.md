...
- Tasks:
        ...
        # Step 8 - Creating the /scripts directory with '0755' permissions
        # Step 9 - Creating the .env file in /scripts directory with '0740' permissions
        # Step 10 - Copying the pgdump_standby_postgresql.py script to ~/scripts with '0755' permissions
        # Step 11 - Filling the .env file with data
        # Step 12 - Copying Private ssh-key to ~/.ssh with '600' permissions
        # Step 13 - Copying the logrotate_pgdump_standby_posgresql file to /etc/logrotate.d/
        # Step 14 - Starting and enabling the logrotate service
        # Step 15 - Adding the execution of pgdump_standby_postgresql.py to the cron job scheduler
...
- Step 11 - Filling the .env file with data
        ...
        DATABASE_PORT=5432
        REMOTE_HOST=192.168.10.13 # the ip address of Primary PostgreSQL
        REMOTE_USER=root
...
- Step 12 - Filling the .env file with data
        ...
        BACKUPS_DIR="/opt/vhdd-1-monitoring-system-db/zabbix_dump"
...
- Step 3 - Removing noip-duc.tar.gz archive from /root/noip-duc.tar.gz
...
- Step 3 - Starting and Enabling nginx services
...
- Step 4 - Starting and Enabling postgresql services
...
- Step 11 - Copying the nginx_mediawiki configuration file to /var/www/mediawiki/LocalSettings.php with '0755' permissions
...
- Step 2 - Downloading the Zabbix repository
        ...
        cd /tmp
        wget https://repo.zabbix.com/zabbix/7.0/ubuntu/pool/main/z/zabbix-release/zabbix-release_latest+ubuntu22.04_all.deb -O /tmp/zabbix_all.deb
...
- Step 7 - Configuring the "DBPassword" in /etc/zabbix/zabbix_server.conf 
        ...
        DBPassword=<ansible_secrets.yaml postgres_zabbix_server_user_vars.db_user_password>
...
- Step 8, 9 - Configuring the "listen" and "server_name" in /etc/zabbix/nginx.conf
        ...
        listen 8080
        # IP or DDNS address
        server_name monitoring-wiki.ddns.net


- Step 10 - Restarting and Enabling services

        systemctl start <servic_name>
        systemctl enable <servic_name>
